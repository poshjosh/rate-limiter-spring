...
final class DefaultPathContainer implements PathContainer {
    ...
    private static final PathContainer EMPTY_PATH = new DefaultPathContainer("", Collections
            .emptyList());

    private static final Map<Character, DefaultPathContainer.DefaultSeparator> SEPARATORS = new HashMap<>(2);

    static {
        SEPARATORS.put('/', new DefaultPathContainer.DefaultSeparator('/', "%2F"));
        SEPARATORS.put('.', new DefaultPathContainer.DefaultSeparator('.', "%2E"));
    }
    ...
    static PathContainer createFromUrlPath(String path, Options options) {
        ...
        DefaultPathContainer.DefaultSeparator separatorElement = SEPARATORS.get(separator);
        ...
        while (begin < path.length()) {
            ...
            if (!segment.isEmpty()) {
                elements.add(options.shouldDecodeAndParseSegments() ?
                        ...
                        new DefaultPathContainer.DefaultPathSegment(segment, separatorElement));
            }
            if (end == -1) {
                break;
            }
    ...
    private static PathSegment decodeAndParsePathSegment(String segment) {
        ...
        if (index == -1) {
            ...
            return new DefaultPathContainer.DefaultPathSegment(segment, valueToMatch, EMPTY_PARAMS);
        }
        else {
    ...
    static PathContainer subPath(PathContainer container, int fromIndex, int toIndex) {
        ...
        assertTrue(fromIndex >= 0 && fromIndex < elements.size(), "Invalid fromIndex: " + fromIndex);
        assertTrue(toIndex >= 0 && toIndex <= elements.size(), "Invalid toIndex: " + toIndex);
        assertTrue(fromIndex < toIndex, "fromIndex: " + fromIndex + " should be < toIndex " + toIndex);
    ...
    private static class DefaultPathSegment implements PathSegment {
        ...
        DefaultPathSegment(String value, String valueToMatch, MultiValueMap<String, String> params) {
            ...
            this.parameters = MultiValueMap.unmodifiable(params);
        }

        /**
         * Constructor for segments without decoding and parsing.
         */
        DefaultPathSegment(String value, DefaultPathContainer.DefaultSeparator separator) {
            this.value = value;
